...
function renderCategoryStats(list) {
  ...
  const totalEl = document.getElementById('cat-stats-total');
  if (totalEl) totalEl.textContent = `${total} empresa${total !== 1 ? 's' : ''} · ${sorted.length} categorias`;

  if (!sorted.length) {
    container.innerHTML = '<p style="color:var(--text-muted);font-size:13px;grid-column:1/-1">Nenhuma empresa cadastrada.</p>';
    return;
  }

  const colors = ['#6366f1','#10b981','#f59e0b','#ec4899','#3b82f6','#8b5cf6','#14b8a6','#f97316','#84cc16','#06b6d4'];
  container.innerHTML = sorted.map(([cat, count], i) => {
    const pct = total > 0 ? Math.round((count / total) * 100) : 0;
    const color = colors[i % colors.length];
    return `
      <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:14px 16px;display:flex;align-items:center;gap:12px;transition:box-shadow .2s" onmouseover="this.style.boxShadow='0 4px 16px rgba(0,0,0,.12)'" onmouseout="this.style.boxShadow='none'">
        ...
        <div style="flex:1;min-width:0">
          <div style="font-size:12px;font-weight:600;color:var(--text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:4px">${cat}</div>
        ...
        <div style="text-align:right;flex-shrink:0">
          <div style="font-size:20px;font-weight:800;color:${color};line-height:1">${count}</div>
...
function getCategoryIcon(cat) {
  const icons = {
    'Alimentação':'🍽️','Beleza e Estética':'💅','Consertos e Reparos':'🔧',
    'Lojas':'🛍️','Material de Construção':'🏗️','Saúde':'🏥','Transporte':'🚗',
    'Aniversário':'🎂','Presentes':'🎁','Empregos':'💼','Grupos':'👥',
    'Tecnologia e Informática':'💻','Educação':'📚','Imóveis':'🏠','Pet Shop':'🐾',
    'Esporte e Lazer':'⚽','Contabilidade e Finanças':'💰','Advocacia e Jurídico':'⚖️',
    'Religião e Igrejas':'⛪','Eventos e Festas':'🎉','Influenciadores':'📱',
    'Artistas':'🎨','Sem categoria':'📋'
  };
...
window.showPage = function(pageId) {
  document.querySelectorAll('.page, .admin-page, section').forEach(page => {
